 ...
 * MA 02111-1307 USA
 *
 * Description:   NUC900 MAC driver source file
 */
...
int nuc900_eth_mii_write(uchar addr, uchar reg, ushort val)
{
...
int nuc900_eth_mii_read(uchar addr, uchar reg, ushort *val)
{
...
int nuc900_reset_phy(void)
{
        ...
        nuc900_eth_mii_write(CONFIG_NUC900_PHY_ADDR, MII_BMCR, BMCR_RESET);
        
        delay = 2000;
        while(delay-- > 0) {
                nuc900_eth_mii_read(CONFIG_NUC900_PHY_ADDR, MII_BMCR, &reg);
                if((reg & BMCR_RESET) == 0)
                        break;
        ...
        nuc900_eth_mii_write(CONFIG_NUC900_PHY_ADDR, MII_ADVERTISE, ADVERTISE_CSMA |
                                                                    ADVERTISE_10HALF |
                                                                    ADVERTISE_10FULL |
                                                                    ADVERTISE_100HALF |
                                                                    ADVERTISE_100FULL);
        ...
        nuc900_eth_mii_read(CONFIG_NUC900_PHY_ADDR, MII_BMCR, &reg);
        nuc900_eth_mii_write(CONFIG_NUC900_PHY_ADDR, MII_BMCR, reg | BMCR_ANRESTART);

        delay = 20000;
        while(delay-- > 0) {
                nuc900_eth_mii_read(CONFIG_NUC900_PHY_ADDR, MII_BMSR, &reg);
        ...
        if(delay == 0) {
                ...
                nuc900_eth_mii_read(CONFIG_NUC900_PHY_ADDR, MII_LPA, &reg);
...
void init_rx_desc(void)
{
        ...
        for(i = 0; i < RX_DESCRIPTOR_NUM; i++) {
                ...
                rx_desc[i].buf = (unsigned int)NetRxPackets[i] | NON_CACHE_ADDR;
                rx_desc[i].status2 = 0;
...
int nuc900_eth_write_hwaddr(struct eth_device *dev)
{       
...
int nuc900_eth_init(struct eth_device *dev, bd_t *bis)
{
        // Reset MAC
        ...
        nuc900_eth_write_hwaddr(dev);  // need to reconfigure hardware address 'cos we just RESET emc...
        
        writel(MCMDR_SPCRC | MCMDR_RXON | MCMDR_EnMDC | MCMDR_TXON, MCMDR);
        writel(0, RSDR);
        
        return(nuc900_reset_phy());

}



void  nuc900_eth_halt(struct eth_device *dev)
{
        writel(readl(MCMDR) & ~(MCMDR_RXON|MCMDR_TXON), MCMDR);
}

int nuc900_eth_recv (struct eth_device *dev)
{
        ...
        while(1) {
                ...
                if(rx_desc_ptr->status1 & RXFD_RXGD) {
                        NetReceive(rx_desc_ptr->buf, rx_desc_ptr->status1 & 0xFFFF);
                }
...
int nuc900_eth_send(struct eth_device *dev, volatile void *packet, int length)
{
...
int nuc900_eth_register(void)
{
        ...
        dev->init = nuc900_eth_init;
        dev->halt = nuc900_eth_halt;
        dev->send = nuc900_eth_send;
        dev->recv = nuc900_eth_recv;
        dev->write_hwaddr = nuc900_eth_write_hwaddr;        
