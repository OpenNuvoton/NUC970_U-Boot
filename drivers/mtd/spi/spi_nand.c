                                                ...
                                                u8 *idcode)
{
	...
	flash->sector_size = params->page_size * params->pages_per_sector;  //ya
...
int spi_nand_init(void)
{
	...
	mtd = &nand_info[/*CONFIG_IPQ_SPI_NAND_INFO_IDX*/0];
	chip = &nand_chip[/*CONFIG_IPQ_SPI_NAND_INFO_IDX*/0];
	...
	if ((ret = nand_register(/*CONFIG_IPQ_SPI_NAND_INFO_IDX*/0)) < 0) {
		free(info);
